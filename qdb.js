...
function home() {
    ...
    d.push({
        title: '',
        img: 'hiker://files/rules/joker/img/豆瓣.png',
        desc: '0',
        col_type: 'card_pic_1',
        url: $('hiker://empty#noHistory')
            ...
            d.push({
                ...
                desc: '小白勿触',
                col_type: 'text_center_1',
                url: $("eval(fetch(getVar('qdb_file')));detailsView(type, id);", '先eval（）你的文件'+'\n'+'再调用你写的详情页面函数'+'\n'+'(函数的传入参数为影片type和id)')
                    .input(() => {
...
function search() {
    ...
    let page = getPage();
    let pageObj = getDoubanRes('https://frodo.douban.com/api/v2/search/subjects?q=' + wd + '&type=movie' + (page ? '&start=' + (page - 1) * 20 : '&start=0') + '&count=20' + '&loc_id=118289&os_rom=android&apikey=0dad551ec0f84ed02907ff5c42e8ec70')
...
function stillsList(type, id) { //剧照页面
    let page = getPage();
    let r = getDoubanRes("https://frodo.douban.com/api/v2/" + type + "/" + id + "/photos?start=" + 30 * (page - 1) + "&count=30" + "&apikey=0dad551ec0f84ed02907ff5c42e8ec70");
...
function elessarWorks(id) { //演职人员详情页面
    let i = getDoubanRes("https://frodo.douban.com/api/v2/elessar/subject/" + id + "?apikey=0dad551ec0f84ed02907ff5c42e8ec70");
    let t = "", r = "";
    i.modules.forEach((e => {
    ...
    i = getDoubanRes("https://frodo.douban.com/api/v2/elessar/work_collections/" + t + "/works?start=" + 30 * (getPage() - 1) + "&count=30&collection_title=" + r + "&apikey=0dad551ec0f84ed02907ff5c42e8ec70");
...
function hotList(page, count) { //热门
    ...
    let s = "", l = "";
    if (u == 'movie') {
        s = getDoubanRes("https://frodo.douban.com/api/v2/movie/hot_gaia" + (page ? "?start=" + (page - 1) * (count || 30) + "&count=" + (count || 30) : "") + "&apikey=0dad551ec0f84ed02907ff5c42e8ec70");
        l = s.items;
    } else {
        s = getDoubanRes("https://frodo.douban.com/api/v2/subject_collection/tv_hot/items" + (page ? "?start=" + (page - 1) * (count || 30) + "&count=" + (count || 30) : "") + "&apikey=0dad551ec0f84ed02907ff5c42e8ec70");
