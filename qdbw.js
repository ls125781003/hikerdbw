...
//预处理
function pre() {
    let file = "hiker://files/rules/joker/qdbw_config.js";
    if (!fetch(file)) {
...
function initConfigs(name, config) {
    if (!config) {
        config = JSON.parse(fetch(getVar('qdbw_config')));
    }
    if (config[name] == null) {
...
function getConfig(name, rootConfig) {
    let config = rootConfig ? rootConfig : JSON.parse(fetch(getVar('qdbw_config')));
    if (name) {
        if (config[name] == null) {
...
function rating(type, id, ratingCount) {
    ...
    let r2 = '';
    s.awards.forEach(e => {
        r2 += (e.ceremony.title + '(' + e.ceremony.year + ')').big().bold() + '<small>(<a href="hiker://empty#noHistory#@rule=js:eval(fetch(getVar(`qdbw_file`)));awardView(`' + e.ceremony.id + '`,`' + e.ceremony.title + '`);">查看详情</a>)</small>' + '<br/>';
...
function dramaReviewList(type, id) {
    ...
    r.reviews.forEach((e => {
        ...
        l.push({
            title: "<strong>" + e.title + "</strong><br/>" + e.abstract + '   <small>(<a href="hiker://empty#noHistory#@rule=js:eval(fetch(getVar(`qdbw_file`)));dramaReviewView(' + e.id + ')">更多</a>)</small>' + (t ? '<br/><small>看过 <font color=' + starColor + '>' + t + "</font></small>" : "") + '<br/><small><font color="grey">' + i + r + o + "</font></small>",
            col_type: "rich_text"
        })
        l.push({
            col_type: "line"
    ...
    i.awards.forEach(t => {
        ...
        for (let i = 0; i < t.modules.length; i++) {
            ...
            if (!e.ceremony || !e.category) {
                ...
                l.push({
                    title: e.ceremony.title.bold() + '<small>(<a href="hiker://empty#noHistory#@rule=js:eval(fetch(getVar(`qdbw_file`)));awardView(`' + e.ceremony.id + '`,`' + e.ceremony.title + '`);">查看详情</a>)</small>' + '<br/>' + (e.category.title + (e.is_won ? '' : '(提名)')).small(),
                    col_type: 'rich_text'
                })
            }
        }
...
function dramaReviewView(id) {
    ...
    l.comments.length > 0 ? l.comments.forEach((e => {
        ...
        o.push({
            title: e.text + (e.replies.length > 0 ? ' <small><a href="hiker://empty#noHistory#@rule=js:eval(fetch(getVar(`qdbw_file`)));dramaReviewReplyView(' + e.id + ');">[查看回复]</a></small>' : ""),
            col_type: "rich_text"
        })
        o.push({
            col_type: "line"
    ...
    let color = JSON.parse(fetch(getVar('qdbw_config')))
        .starColor || "#ffac2d";
    let s = [{
...
function detailsView(type, id) {
    ...
    let relatedItems = [{
        title: "剧照",
        img: "https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/img/剧照.png",
        ...
        title: "演职",
        img: "https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/img/演职.png",
        url: $('hiker://empty/#/$page{fypage}#noHistory#').rule((t) => {
            eval(fetch(getVar("qdbw_file")));
            credits(t[0], t[1]);
        }, [i.subtype, i.id]),
        ...
        title: "短评",
        img: "https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/img/短评.png",
        url: $('hiker://empty/#/$page{fypage}#noHistory#').rule((t) => {
            eval(fetch(getVar("qdbw_file")));
        ...
        title: "剧评",
        img: "https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/img/剧评.png",
        url: $('hiker://empty/#/$page{fypage}#noHistory#').rule((t) => {
            eval(fetch(getVar("qdbw_file")));
    ...
    if (i.trailer) {
        ...
        videoRelatedItems.push({
            title: '查看更多',
            img: "https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/img/more.png",
            col_type: "movie_2",
            url: $('hiker://empty/#/$page{fypage}#noHistory#').rule((t) => {
                eval(fetch(getVar("qdbw_file")));
                trailers(t[0], t[1]);
...
function classList(page, count) {
    ...
    let items = {
        ...
        year: ["全部年代","2024", "2023", "2022", "2021", "2020", "2019", "2010年代", "2000年代", "90年代", "80年代", "70年代", "60年代", "更早"],
...
function modeEditPage(d, configKey, params) {
    ...
    d.push({
        ...
        url: $().lazyRule((configKey, params) => {
            let config = JSON.parse(request(getVar('qdbw_config')))
        ...
        d.push({
            ...
            url: $("hiker://empty#noLoading#").lazyRule(configKey => {
                ...
                if (config.detailsViewConfigs.use === configKey) {
                    ...
                    return 'hiker://page/setting-editor?rule=轻豆瓣网'
                }
...
function homePageModeEditPage(d) {
    d.push({
        title: '<font color="red">' + '返回数组,供setResult使用' + '</font>' + '<font color="#808080"><br>' + '【内置函数】' + '<br>&nbsp;&nbsp;&nbsp;settingPage(): 设置页(默认首页有使用示例)' + '</font>',
        col_type: 'rich_text'
    })
        ...
        d.push({
            ...
            url: $("hiker://empty#noLoading#").lazyRule(configKey => {
                ...
                if (config.homePageConfigs.use === configKey) {
                    putVar("input_config_type", '首页配置')
                    putVar("input_name", configKey)
                    putVar("input_code", config.homePageConfigs[configKey].config)
                    return 'hiker://page/setting-editor?rule=轻豆瓣网'
                }
...
let ConfigTool = {
    ...
    export: (name, config, remark, encType) => {
        let symbol = "轻豆瓣网口令￥" + remark + "￥" + name + "￥" + encType + "￥";
        let result = config
    ...
    import: (configCommand) => {
        ...
        if (configCommand.startsWith('http')) {
            if (getAppVersion() > 2070) {
                result = configCommand.split('轻豆瓣网')[0].replace(/\\n/, '')
...
function analysisVerifyModule(d) {
    if (false/*getItem('password') !== "true"*/) {
        d.push({
            title: '点我验证身份',
            col_type: 'text_center_1',
            url: $('', '此规则用于哪款软件？').input(() => {
...
function settingPage() {
    //eval(fetch(getVar('qdbw_file')));
    let conf = JSON.parse(fetch(getVar('qdbw_config')));
    let d = [];
    ...
    d.push({
        ...
        url: $('hiker://empty').lazyRule((currentVersion) => {
            let gitfile = request("https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/qdbw.js");
            eval(gitfile);
            if (version > currentVersion) {
                return $("发现新版本,是否更新?").confirm((gitfile) => {
                    writeFile("hiker://files/rules/joker/qdbw.js", gitfile);
                    back(true);
            ...
            d.push({
                ...
                url: $('hiker://empty?id=' + id + '&type=' + type + '&title=' + e.title).rule((res, id, e, lazy, extra) => {
                    ...
                    if (typeof(urls) == "object") {
                        ...
                        let rule = JSON.parse(request("hiker://page/releaseIP?rule=轻豆瓣网"))
                            .rule;
                        eval(rule);
