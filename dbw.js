...
extra: {ua: MOBILE_UA}
    ...
    d.push({
        title: '⚙️设置',
        //img: 'https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/img/豆瓣.png',
        img: 'https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/lmtubiao/main/erdi/64.png',
        col_type: 'text_icon',
    ...
    i.awards.forEach(t => {
        ...
        for (let i = 0; i < t.modules.length; i++) {
            ...
            if (!e.ceremony || !e.category) {
                ...
                l.push({
                    title: e.ceremony.title.bold() + '<small>(<a href="hiker://empty#noHistory#@rule=js:eval(fetch(getVar(`dbw_file`)));awardView(`' + e.ceremony.id + '`,`' + e.ceremony.title + '`);">查看详情</a>)</small>' + '<br/>' + (e.category.title + (e.is_won ? '' : '(提名)')).small(),
                    col_type: 'rich_text'
                })
            }
        }
...
function dramaReviewView(id) {
    ...
    l.comments.length > 0 ? l.comments.forEach((e => {
        ...
        o.push({
            title: e.text + (e.replies.length > 0 ? ' <small><a href="hiker://empty#noHistory#@rule=js:eval(fetch(getVar(`dbw_file`)));dramaReviewReplyView(' + e.id + ');">[查看回复]</a></small>' : ""),
            col_type: "rich_text"
        })
        o.push({
            col_type: "line"
    ...
    let color = JSON.parse(fetch(getVar('dbw_config')))
        .starColor || "#ffac2d";
    let s = [{
...
let ConfigTool = {
    ...
    export: (name, config, remark, encType) => {
        let symbol = "豆瓣网口令￥" + remark + "￥" + name + "￥" + encType + "￥";
        let result = config
    ...
    import: (configCommand) => {
        ...
        if (configCommand.startsWith('http')) {
            if (getAppVersion() > 2070) {
                result = configCommand.split('豆瓣网')[0].replace(/\\n/, '')
...
function settingPage() {
    ...
    d.push({
        ...
        url: $('hiker://empty').lazyRule((currentVersion) => {
            let gitfile = request("https://ghproxy.net/https://raw.githubusercontent.com/ls125781003/hikerdbw/master/dbw.js");
            eval(gitfile);
            if (version > currentVersion) {
                return $("发现新版本,是否更新?").confirm((gitfile) => {
                    writeFile("hiker://files/rules/joker/dbw.js", gitfile);
                    back(true);
            ...
            d.push({
                ...
                url: $('hiker://empty?id=' + id + '&type= ' + type + '&title=' + e.title).rule((res, id, e, lazy, extra) => {
                    ...
                    if (typeof(urls) == "object") {
                        ...
                        let rule = JSON.parse(request("hiker://page/releaseIP?rule=豆瓣网"))
                            .rule;
                        eval(rule);
